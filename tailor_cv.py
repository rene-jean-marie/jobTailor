...
def build_output_dir_name(company: str, role: str, created: str) -> str:
    company_part = slugify_token(company)
    role_part = slugify_token(role)
    date_part = slugify_token(created)
    return f"{company_part}_{role_part}_{date_part}"
...
def process_job(
    ...
    if make_pdf:
        ...
        created_paths.extend([cv_pdf_path, cover_pdf_path])

    return created_paths


def load_job_texts(
    urls: List[str], job_text_file: Path | None
) -> List[Tuple[str, str]]:
    if job_text_file:
...
def main() -> int:
    ...
    if not args.job_url and not args.job_text_file:
        raise SystemExit("Provide --job-url or --job-text-file")

    cv_text = load_cv_text(Path(args.cv_file))
    out_dir = Path(args.out_dir)
    job_text_file = Path(args.job_text_file) if args.job_text_file else None

    jobs = load_job_texts(args.job_url or [], job_text_file)

    for source, job_text in jobs:
        slug = slugify(source)
        created_paths = process_job(
            cv_text=cv_text,
            job_text=job_text,
            out_dir=out_dir,
            slug=slug,
